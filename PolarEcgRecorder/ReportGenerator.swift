...
class ReportGenerator {
    ...
    private static func buildPDF(
        ...
        let data = renderer.pdfData { ctx in
            ...
            attr("Zaznamenané: \(df.string(from: group.sessionDate))   •   Trvanie: \(fmtDur(duration))   •   Zariadenie: Polar H10",
                 font: .systemFont(ofSize: 10), color: .secondaryLabel)
                .draw(at: CGPoint(x: M, y: y)); y += 16

            attr("Vygenerované aplikáciou PolarECG Monitor",
            ...
            // Stats grid
            sectionTitle("Súhrn merania", ctx: cgc, y: &y, x: M, w: CW)
            let stats: [(String, String, String)] = [
                ("Priemerná TF",     fmtD(avgHR, dec: 0), "BPM"),
                    ("Min. TF",          "\(Int(minHR))",     "BPM"),
                    ("Max. TF",          "\(Int(maxHR))",     "BPM"),
                    ("Priemerný RMSSD",  avgHRV > 0 ? fmtD(avgHRV) : "N/A", "ms"),
                    ("Min. RMSSD",       minHRV > 0 ? fmtD(minHRV) : "N/A", "ms"),
                    ("Max. RMSSD",       maxHRV > 0 ? fmtD(maxHRV) : "N/A", "ms"), // 👈 PRIDANÉ
                    ("Počet udalostí",   "\(eventIndices.count)", "")            ]
            y = drawStatsGrid(stats: stats, ctx: cgc, y: y, x: M, cw: CW)
            y += 14

            // HRV interpretation box
            if avgHRV > 0 {
                let interp = hrvInterpretation(avgHRV)
                drawInfoBox(text: "HRV interpretácia: \(interp)",
            ...
            // HR trend chart
            if hr.count > 1 {
                sectionTitle("Trend srdcovej frekvencie", ctx: cgc, y: &y, x: M, w: CW)
            ...
            // HRV trend chart
            if hrv.count > 1 {
                sectionTitle("Trend HRV (RMSSD)", ctx: cgc, y: &y, x: M, w: CW)
            ...
            for (evNum, evIdx) in eventIndices.enumerated() {
                ...
                if hr.count > 1 {
                    ey += 4
                    sectionTitle("Srdcová frekvencia počas záznamu", ctx: cgc2, y: &ey, x: M, w: CW)
            ...
            if eventIndices.isEmpty {
                ctx.beginPage()
                var ny = M
                attr("Žiadne udalosti neboli zaznamenané počas tohto záznamu.",
                     font: .systemFont(ofSize: 13), color: .secondaryLabel)
                    .draw(at: CGPoint(x: M, y: ny)); ny += 20
                attr("Tlačidlo MARK EVENT nebolo stlačené.",
    ...
    private static func drawECGStrip(
        ...
        let minV = -300.0, maxV = 900.0, range = maxV - minV
    ...
    private static func hrvInterpretation(_ rmssd: Double) -> String {
        switch rmssd {
        case ..<20:  return "Nízky HRV (< 20 ms) — zvýšená záťaž ANS, únava alebo patológia"
        case ..<50:  return "Normálny HRV (20–50 ms) — bežný rozsah pre dospelého"
        default:     return "Vysoký HRV (> 50 ms) — dobrá variabilita, zdravý ANS"
        }
    }
