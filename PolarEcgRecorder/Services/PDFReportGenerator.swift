...
public class PDFReportGenerator {
    ...
    public static func generate(group: SessionGroup, summary: SessionSummary, completion: @escaping (Result<URL, Error>) -> Void) {
        DispatchQueue.global(qos: .userInitiated).async {
            var ecg = ECGAnalyzer.parseECG(group.ecgURL)
            var hr  = ECGAnalyzer.parseHR (group.hrURL)
            var hrv = ECGAnalyzer.parseHRV(group.hrvURL)
            
            do {
                try ECGAnalyzer.validateDataIntegrity(ecg: &ecg, hr: &hr, hrv: &hrv)
            } catch {
    ...
    private static func buildPDF(
        ...
        let data = renderer.pdfData { ctx in
            var currentPage = 1
            func addNewPage() {
                PDFReportGenerator.drawFooter(ctx: ctx.cgContext, pageNum: currentPage)
                currentPage += 1
                ctx.beginPage()
            }
